...
#' h = 0.1, propose a new point, and check the new implausibility. If the new implausibility
...
#'
#' @export
directional_proposal <- function(ems, x, targets, accept = 2, hcutoff = 1e-06, iteration.steps = 50) {
  ...
  test_dirs <- runifs(1000, length(x))
  ...
  while(attempts < 50) {
    ...
    nimp <- nth_implausible(ems, new_point, targets)
    if (nimp < old_imp) {
      better_pt <- new_point
      old_imp <- nimp
      index <- index + 1
