  ...
  public = list(
    ...
    initialize = function(basis_f, beta, u, ranges, bucov = NULL, data = NULL, delta = 0, model = NULL) {
      ...
      self$active_vars <- purrr::map_lgl(seq_along(ranges), function(x) {
        is_in_func <- eval_funcs(self$basis_f, matrix(c(rep(0,x-1), 1, rep(0, length(ranges)-x)), nrow=1)) == 1
        length(is_in_func[is_in_func]) > 1
        })
      self$corr_func <- function(x, xp) {
        if (sum((x-xp)^2) < 0.0001) extra <- 1 else extra <- 0
        (1-delta) * u$corr(x[self$active_vars], xp[self$active_vars]) + delta * extra
      }
      self$theta <- sqrt(-0.25/(log(u$corr(0, 0.5)) - log(1-delta)))
      if (is.null(bucov)) self$beta_u_cov <- function(x) rep(0, length(self$beta_mu)) else self$beta_u_cov <- bucov
      ...
      {
        if (class(self$in_data) != "data.frame" && length(dim(self$in_data)) == 0) self$in_data <- matrix(self$in_data, nrow = length(self$in_data))
        private$data_corrs <- chol2inv(chol(self$u_sigma^2 * apply(self$in_data, 1, function(y) eval_funcs(self$corr_func, self$in_data, y))))
        private$design_matrix <- eval_funcs(self$basis_f, self$in_data)
    ...
    get_cov = function(x, xp = NULL, full = FALSE) {
      x <- eval_funcs(scale_input, x, self$ranges)
      if (is.null(xp)) xp <- x else xp <- eval_funcs(scale_input, xp, self$ranges)
      if (class(x) != "data.frame" && length(dim(x)) == 0) x <- matrix(x, nrow = length(x))
      if (class(xp) != "data.frame" && length(dim(xp)) == 0) xp <- matrix(x, nrow = length(xp))
      g_x <- eval_funcs(self$basis_f, x)
      g_xp <- eval_funcs(self$basis_f, xp)
      if (full) {
        x_xp_c <- apply(xp, 1, function(y) eval_funcs(self$corr_func, x, y))
        beta_part <- g_x %*% self$beta_sigma %*% t(g_xp)
        u_part <- self$u_sigma^2 * x_xp_c
        bupart_x <- t(eval_funcs(self$beta_u_cov, x))
        bupart_xp <- t(eval_funcs(self$beta_u_cov, xp))
        if (!is.null(self$in_data)) {
          ...
          if (length(dim(c_x)) != 0) c_xt <- t(c_x) else c_xt <- c_x
          if (length(dim(c_xp)) != 0) c_xpt <- t(c_xp) else c_xpt <- c_xp
          u_part <- u_part - self$u_sigma^2 * c_x %*% (private$data_corrs - private$u_var_modifier) %*% c_xpt * self$u_sigma^2
          bupart_x <- bupart_x - private$beta_u_cov_modifier %*% (private$design_matrix %*% bupart_x + self$u_sigma^2 * c_xt)
          bupart_xp <- bupart_xp - private$beta_u_cov_modifier %*% (private$design_matrix %*% bupart_xp + self$u_sigma^2 * c_xpt)
        }
        bupart <- g_x %*% bupart_xp + t(bupart_x) %*% t(g_xp)
      }
      else {
        if (length(seq_along(x[,1])) != length(seq_along(x[,2]))) stop("Cannot compute diagonal elements of a non-square covariance matrix.")
        xseq <- seq_along(x[,1])
        beta_part <- purrr::map_dbl(xseq, ~g_x[.,] %*% self$beta_sigma %*% g_xp[.,])
        ...
        bupart <- purrr::map_dbl(xseq, ~g_x[.,] %*% bupart_xp[,.x] + t(bupart_x[,.x]) %*% g_xp[.,])
      }
      return(beta_part + u_part + bupart)
    },
    implausibility = function(x, z) {
      if (is.numeric(z)) output <- list(val = z, sigma = 0) else output <- z
    ...
    adjust = function(data, out_name) {
      ...
      if (class(this_data_in) != "data.frame" && length(dim(this_data_in)) == 0) this_data_in <- matrix(this_data_in, nrow = length(this_data_in))
      this_data_out <- data[, out_name]
      G <- eval_funcs(self$basis_f, this_data_in)
      ...
      new_em <- Emulator$new(self$basis_f, list(mu = new_beta_exp, sigma = new_beta_var),
                             ...
                             bucov = self$beta_u_cov, ranges = self$ranges,
