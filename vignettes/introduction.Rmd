...
We note that not much of any interest is going on here. The emulator variance is stationary (i.e. independent of position), and the expectation is simply given by a linear relationship between the two variables in question. Before we train the emulators, we will add more structure to the initial emulators.

We can add a 'nugget' terms to the emulators. The idea is that we should encode some uncertainty into the results from the fitted points (those from `GillespieSIR`): the overall covariance structure will instead be `sigma^2*(1-delta)*exp_sq + delta*sigma^2*I`, where `I` is an indicator function and `delta` a numeric parameter for each output. The `sigma` terms are unchanged from the basic emulators.

```{r}
nugget_emulators <- emulator_from_data(input_data, input_names, output_names, ranges = ranges, deltas = c(0.08522, 0.00604, 0.04123))
```

The emulator expectation is unchanged, but the covariance structure is.
...
We won't put this in a code block, as it takes a while to run and the output is not instructive in an example, but the code structure would be `next_runs <- gen_new_runs(GillespieSIR[, input_names], trained_emulators, ranges, n_points = 20, n_runs = 100, z)` to generate 20 new points, allowing for 100 different designs over which the optimal design can be chosen.
